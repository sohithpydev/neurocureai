...
# ==========================================
# GIF RENDERING HELPER (Prevents freezing)
# ==========================================
def render_animated_gif(file_path):
    """Encodes GIF to Base64 to force browser animation playback"""
    if os.path.exists(file_path):
        with open(file_path, "rb") as f:
            data = f.read()
            encoded = base64.b64encode(data).decode()
        st.markdown(
            f'<img src="data:image/gif;base64,{encoded}" style="width:100%; border-radius:20px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">',
            unsafe_allow_html=True
        )
        return True
    return False
...
with tab_home:
    ...
    with col2:
        ...
        with col2_1:
            # Using Base64 to force Hero GIF to play
            if not render_animated_gif("media/hero_brain_ai.gif"):
    ...
    # FIX: Forcing portfolio.gif to animate via Base64
    if os.path.exists("media/portfolio.gif"):
        render_animated_gif("media/portfolio.gif")
    elif os.path.exists("media/portfolio_2.gif"):
        render_animated_gif("media/portfolio_2.gif")
